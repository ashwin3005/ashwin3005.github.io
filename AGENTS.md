...
**Dark mode is handled entirely via CSS.** The `[data-theme="dark"]` selector on `<html>` overrides all token values. Components do not need JS or class-toggling — they just use the tokens.
